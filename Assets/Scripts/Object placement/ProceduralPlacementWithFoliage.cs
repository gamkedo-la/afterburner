...
public class ProceduralPlacementWithFoliage : MonoBehaviour
{
    ...
    [Header("Foliage")]
    [SerializeField] PlacementOptionsFoliage[] m_foliageOptions;
    ...
    private List<Vector3> m_foliageObjectPositions = new List<Vector3>();
	...
	private void PlaceFoliage()
	{
		...
		for (int k = 0; k < m_foliageOptions.Length; k++)
		{
			var options = m_foliageOptions[k];
			int number = options.number;
			var foliageTypePrefabs = options.foliageTypePrefabs;
			...
			for (int i = 0; i < number; i++)
			{
				int index = Random.Range(0, foliageTypePrefabs.Length);
				var foliageGameObject = (GameObject)Instantiate(foliageTypePrefabs[index].gameObject, Vector3.zero, Quaternion.identity);
				var foliageObject = foliageGameObject.GetComponent<PlaceableObjectFoliage>();
				foliageGameObject.gameObject.transform.parent = transform;
				...
				while (!success && attempts <= m_maxPlacementAttempts)
				{
					success = TestFoliagePosition(foliageObject, options);
					attempts++;
				}
				...
				{
					//print(string.Format("Failed to place enemy ground defence number {0} ({1}) in wave {2}", i + 1, groundGameObject.name, k + 1));
					Destroy(foliageGameObject);
				}
				else
				{
					float scale = Random.Range(options.minScale, options.maxScale);
					foliageGameObject.transform.localScale = new Vector3(scale, scale * Random.Range(1, options.maxYStretch), scale);
				}
				//                else
				//                  m_missionGoals.AddGroundObject(groundGameObject.transform);
			}
    ...
    {
		Debug.Log("testFP" + testPlaceableObject.ToString());
