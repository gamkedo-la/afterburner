...
public class ProceduralPlacement : MonoBehaviour
{
	...
	[Header("Options")]
	[SerializeField]
	bool m_showDebugSpheres = false;
	[SerializeField]
	bool m_useGlobalSeed = true;
	[SerializeField]
	int m_seed = 1;
	[SerializeField]
	int m_maxPlacementAttempts = 100;

	[Header("Main target")]
	[SerializeField]
	PlaceableObject m_mainTargetPrefab;
	[SerializeField]
	float m_minDistanceFromPlayer = 800f;
	[SerializeField]
	float m_maxDistanceFromPlayer = 1200f;
	[SerializeField]
	float m_minAngleFromNorth = -45f;
	[SerializeField]
	float m_maxAngleFromNorth = 45f;
	[SerializeField]
	Vector2 m_groundObjectHeightMinMax = new Vector2(0f, 1000f);
	[SerializeField]
	Vector2 m_airObjectAltitudeMinMax = new Vector2(100f, 500f);

	[Header("Enemy aircraft")]
	[SerializeField]
	PlacementOptionsAir[] m_aircraftPlacementOptions;

	[Header("Enemy ground defences")]
	[SerializeField]
	PlacementOptionsGround[] m_groundDefencesOptions;
	...
	private List<Vector3> m_clutterObjectPositions = new List<Vector3>();
	...
	private void PlaceEnemyGroundDefences()
	{
		...
		for (int k = 0; k < m_groundDefencesOptions.Length; k++)
		{
			...
			int number = options.number;
			var groundDefenceTypePrefabs = options.groundTypePrefabs;
	...
	{
		if (m_clutterOptions.Length == 0)
		{
			return;
		}

		Random.InitState(m_seed + 4);

		for (int k = 0; k < m_clutterOptions.Length; k++)
		{
			var options = m_clutterOptions[k];
			int number = options.number;
			var clutterTypePrefabs = options.groundTypePrefabs;
			...
			for (int i = 0; i < number; i++)
			{
				int index = Random.Range(0, clutterTypePrefabs.Length);
				var clutterGameObject = (GameObject)Instantiate(clutterTypePrefabs[index].gameObject, Vector3.zero, Quaternion.identity);
				var clutterObject = clutterGameObject.GetComponent<PlaceableObjectClutter>();
				clutterGameObject.gameObject.transform.parent = transform;
				...
				while (!success && attempts <= m_maxPlacementAttempts)
				{
					success = TestGroundPosition(clutterObject, options);
					attempts++;
				}
				...
				{
					//print(string.Format("Failed to place enemy ground defence number {0} ({1}) in wave {2}", i + 1, groundGameObject.name, k + 1));
					Destroy(clutterGameObject);
				}
				else
				{
					float scale = Random.Range(options.minScale, options.maxScale);
					clutterGameObject.transform.localScale = new Vector3(scale, scale * Random.Range(1, options.maxYStretch), scale);
				}
			}
		}
	}
			...
			PlacementOptionsGround options, bool mainTarget = false)
	{
		...
		Quaternion trialRotation = Quaternion.identity;
		if (options != null && options.allignWithTerrain)
		{
			trialRotation = Quaternion.LookRotation(m_mapGenerator.GetTerrainNormal(trialPosition.x, trialPosition.z));
			trialRotation *= Quaternion.AngleAxis(90, Vector3.right);
		}
		float rotationY = Random.Range(0f, 360f);
		trialRotation *= //Quaternion.Euler(0, rotationY, 0);
		Quaternion.AngleAxis(rotationY, Vector3.up);
		...
		if (bounds != null)
		{
			...
			float y = m_mapGenerator.getRealHeight(trialPosition.x, trialPosition.z);//rigidbody == null || rigidbody.useGravity == false
	...
	//TODO merge with TestGroundPosition
	/*	private bool TestFoliagePosition(PlaceableObjectFoliage testPlaceableObject,
			PlacementOptionsFoliage options, bool mainTarget = false)
		{
			var testObject = testPlaceableObject.gameObject;
			List<Vector3> objectsToAvoid = mainTarget ? null : m_foliageObjectPositions;
			objectsToAvoid.AddRange(m_groundObjectPositions);
			...
			if (bounds != null)
			{
				...
				if (success)
				{
					...
					if (!mainTarget)
						m_foliageObjectPositions.Add(trialPosition);
